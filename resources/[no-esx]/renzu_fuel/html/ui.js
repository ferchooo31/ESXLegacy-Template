$(document).ready(function(){
  window.addEventListener('message', function( event ) {      
    if (event.data.action == true) {
      ...
      $('#stock').empty();
      if (dataGas.stock == 0) {
        $('#stock').append("<b>FUERA DE STOCK<BR>" + "SIN PETROLEO</b>");
        $('.act').fadeOut();
      } else {
        $('#stock').append("<b>Combusible Disponible:<BR>" + dataGas.stock + " Litros</b>");
        $('.act').fadeIn(300);
      }
  ...
  $( "#retirar" ).click(function() {
    if (!counting) {
      ...
      completar = !completar;
      maxFuel = 99
      $('.modal span').text("Quieres llenar el tanque por $"+(perc_new*dataGas.price).toFixed(2));  
      $('.modal').fadeIn(300);
      $('.container').fadeOut();
      $('left').fadeOut();
      $('.act').fadeOut();
    ...
    if (!counting) {
      ...
      if (price > dataGas.price) {  
        ...
        if (price <= ((100-Math.round(datafuel))*dataGas.price)) {
          ...
          $('.modal span').text("Quieres llenar el tanque por $ "+price); 
          $('.modal').fadeIn(300);
          $('.container').fadeOut();
          $('left').fadeOut();
          $('.act').fadeOut();
        } else {
          var texto = "La cantidad máximo para llenar el tanque es de <b>$"+((100-Math.round(datafuel))*dataGas.price)+"</b>!"
          $.post('http://renzu_fuel/notifytext', JSON.stringify({ text: texto }));
        }
      } else {
        var texto = "El precio debe ser mayor que $" + dataGas.price + "!"
        $.post('http://renzu_fuel/notifytext', JSON.stringify({ text: texto }));
      }
    }
  });
